...
#include "sensor_msgs/msg/joint_state.hpp" // Make sure to include this
...
private:
	...
	// Thread to handle the servo control logic
	...
	// Thread to handle the servo control logic
	// std::thread servo_control_thread_;
	//[(P1-P0)/V]*1000+[V/(A*100)]*1000
	// This function will handle servo control and sleep
	void servoControlLoop()
	{
		// while (rclcpp::ok())
		// {
			// // ((((P2 - P1) / speed) * 1000) + (speed / (acc * 100)) * 1000) * 1000
	...
	void readAndPublishServoData()
	{
		...
		for (uint8_t i = 0; i < sizeof(ID); i++)
		{
			...
			if (Position <= 2047)
				// position_msg.point.x = mapRange(Position, 0, 2047, 0, M_PI);
			...
			// position_msg.point.x = mapRange(Position, 2048, 4095, -M_PI, 0);

			joint_state_msg.name.push_back("base_lidar_joint");
			joint_state_msg.header.stamp = this->get_clock()->now();
			// position_msg.point.y = 0.0; // You can set y and z values as needed, if relevant
			// position_msg.point.z = 0.0;
			// position_msg.header.frame_id = "base_lidar_joint";

			// RCLCPP_INFO(this->get_logger(), "Publishing: '%f'", position_msg.point.x);
			// publisher_->publish(position_msg);

			// RCLCPP_INFO(this->get_logger(), "Publishing joint state: '%f'", joint_state_msg.position[0]);
